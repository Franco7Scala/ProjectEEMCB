...
for output_selected in range(0, output_quantity):
    ...
    forest = RandomForestRegressor(criterion="mse", n_estimators=20, min_samples_split=4, min_weight_fraction_leaf=0.01)
    t0 = time.time()
    ...
    train_sizes_re, train_scores_tree_re, test_scores_tree_re = learning_curve(forest, X[:train_size], y[:train_size], train_sizes=numpy.linspace(0.1, 1, 10), scoring=make_scorer(
