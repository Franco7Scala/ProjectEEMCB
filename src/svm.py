...
for output_selected in range(0, output_quantity):
    ...
    train_sizes_mse, train_scores_svr_mse, test_scores_svr_mse = learning_curve(svr, X[:train_size], y[:train_size], train_sizes=numpy.linspace(0.1, 1, 10), scoring="neg_mean_squared_error", cv=10)
    train_sizes_r2, train_scores_svr_r2, test_scores_svr_r2 = learning_curve(svr, X[:train_size], y[:train_size], train_sizes=numpy.linspace(0.1, 1, 10), scoring="r2", cv=10)
    train_sizes_re, train_scores_svr_re, test_scores_svr_re = learning_curve(svr, X[:train_size], y[:train_size], train_sizes=numpy.linspace(0.1, 1, 10), scoring=make_scorer(scoring.relative_error), cv=10)
