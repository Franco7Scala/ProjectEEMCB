...
def gradient_descent(train_elements, alpha, numIterations, k, verbose = 0):
    x = train_elements[0].input
    y = train_elements[0].output
    m, n = numpy.shape(x)
    theta = numpy.ones(n)
    x_trans = x.transpose()
    for i in range(0, numIterations):
        hypothesis = knn.get_error_estimation_weighted_on_input(train_elements[0].input, theta, train_elements[0].neighbors_i, train_elements[0].neighbors_o, k, False)
        loss = hypothesis - y
        if verbose:
...
if __name__ == '__main__':
    ...
    # loading data
    path_training_set = "/Users/francesco/Desktop/Cose da Sistemare/datas/ds/training_set_wp.txt"
